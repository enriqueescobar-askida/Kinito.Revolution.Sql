...
require("tibble");
# class
SqlToXlsSqlServerInstanceDbPrincipalKeyList <- R6Class("SqlToXlsSqlServerInstanceDbPrincipalKeyList",
  ...
  public = list(
    ...
    getTableNameFrequencyHistogram = function(){
      ...
      if(!is.null(private$Tibble) && dim(private$Tibble)[1]!=0 && dim(private$Tibble)[2]!=0){
        ...
        xTitle <- colnames(t)[1];
        yTitle <- colnames(t)[2];
        mainTitle <- paste0(private$Instance, " PrincipalKey TableName Frequency Histogram");
        colnames(t) <- NULL;
        names(t)[1] <- "X";
        names(t)[2] <- "Y";
        # graph

...
SqlToXlsSqlServerInstanceFactory <-
  R6Class("SqlToXlsSqlServerInstanceFactory",
  portable = TRUE,
  class = TRUE,
  cloneable = TRUE,
  public = list(
    ...
    getLinkedList = function(){
      aList <-
        SqlToXlsSqlServerInstanceLinkedList$new(private$Path, private$SqlServiceInstance, private$SqlServerInstance);
      aList$fileToTibble();
    ...
    getRunningList = function(){
      aList <-
        SqlToXlsSqlServerInstanceRunningList$new(private$Path, private$SqlServiceInstance, private$SqlServerInstance);
      aList$fileToTibble();
      aTibble <- aList$getTibble();
      rm(aList);
      return(tibble::as_tibble(aTibble));
